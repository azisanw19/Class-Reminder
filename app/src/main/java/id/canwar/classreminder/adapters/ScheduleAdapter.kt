...
class ScheduleAdapter(val context: Context, private val schedules: ArrayList<Schedule>) : RecyclerView.Adapter<ScheduleAdapter.ViewHolder>() {
    ...
    open inner class ViewHolder(private val view: View) : RecyclerView.ViewHolder(view){
        ...
        fun bind(schedule: Schedule) {
            ...
            view.schedule_list_day.text = Formatter.getDayFromInt(schedule.day)

            val scheduleTime = "${Formatter.getTimeFromMinute(schedule.timeStart)} - ${Formatter.getTimeFromMinute(schedule.timeEnd)}"
